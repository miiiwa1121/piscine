...
char	put_three_digit_number(char *str)
{
	...
	if (reminder > 0)
	{
		ft_strncpy(splited_triplets[index], str, remainder); //配列の先頭に3で割ったあまりの文字数を代入している
		splited_triplets[index][remainder] = '\0'; //配列の情報にNULLで区切りを付ける
		index++;
	}
	i = remainder;
	while (i < digit_count)
...
int	put_unit_count(char *str)
{
	...
	digit_count = ft_strlen(str);
	unit_count = 0
	while (digit_count > 3)	//3桁の数字はthousand以上の単位を必要としないため
	{
		digit_count -= 3;
		unit_count++;
	}
...
int	main(void)
{
	...
	serch_words_by_key(number, dictionary);
	return (0);
}
